...
locals {
  ...
  alerts_logstash_pipeline_events_out = var.alert_logstash_pipeline_events_default_disabled ? var.alerts_logstash_pipeline_events_out : [
    {
      suffix                = "pager"
      threshold             = "1"
      trigger_after_minutes = 60
...
resource "sysdig_monitor_alert_metric" "logstash_pipeline_events_out" {
  for_each = {
    for index, alert in local.alerts_logstash_pipeline_events_out :
    alert.suffix => alert  if length(alert.channels) > 0
  }
